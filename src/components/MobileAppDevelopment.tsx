...
const Modal = ({project,onClose,}: {project: Project;onClose: () => void;}) => {
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-[var(--footerbg)] bg-opacity-50 flex justify-center items-center z-50">
      <div className="flex flex-col lg:flex-row bg-[var(--secondbackground)] rounded-2xl relative lg:h-[80%] lg:w-[60%] modalcard_shadow">
        <button
        ...
        {/* Project Details */}
        <div className="w-full flex flex-row container mx-auto px-2 py-5">
          {/* for image */}
          <div className="flex flex-1 p-2 items-center justify-center">
            <img
              src={project.image}
              className="w-[200px] lg:rounded-2xl rounded-lg"
            />
          </div>
